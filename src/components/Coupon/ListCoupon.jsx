        ...
        <table className="min-w-full ">
          <thead className="border-b-2 border-gray-300">
            <tr>
              ...
              <th className="px-6 py-3  text-left text-sm font-semibold text-gray-600 tracking-wider">Discount Value</th>
            ...
            {coupons?.length > 0 && coupons?.map((coupon) => (
              <tr key={coupon._id} className="border-b border-gray-300">
                ...
                <td className="px-6 py-4 flex gap-2">
                  <button className="text-gray-600 hover:gray-red-900" onClick={() => handleDeleteCoupon(coupon)}>
                    <FaTrash />
                  </button>
                </td>
              </tr>
      ...
      {showDeleteConfirmation && (
        <div className="fixed inset-0 flex items-center justify-center bg-transparent px-4 md:px-2 lg:px-0 bg-opacity-50 z-50">
          <div className="bg-gray-100 p-8 rounded-lg shadow-lg max-w-md border-gray-300 border">
            <p className="text-lg text-center mb-4">Are you sure to delete this coupon?</p>
